...
#     sst_criterion = nn.CrossEntropyLoss(reduction='mean')
#     para_criterion = nn.BCEWithLogitsLoss(reduction='mean')
#     sts_criterion = nn.MSELoss(reduction='mean')
...
#                     grads = []
#                     for task_loss in [loss_sst, loss_para, loss_sts]:
#                         if task_loss.grad is not None and param in task_loss.grad:
#                             grads.append(task_loss.grad[param])
#                         else:
#                             grads.append(torch.zeros_like(param)) # Fill in a zero gradient if there is no gradient from a task for this parameter
